...
import { patchAd } from "../../services/admin/apiMethods";
import { getAds } from "../../services/apiMethods";
...
const AdminAds = () => {
  ...
  const handleDeleteStatus = async (id, curValue) => {
    try {
      ...
      setLoading(true);
      const response = await patchAd(data)
            ...
            {searchData.map((user1, index, array) => {
              ...
              return (
                <tr key={user._id} className="bg-white text-center">
                  ...
                  <td className="py-2 px-4 border-b">{user?.payment===true ? "Successful":"Pending..."}</td>
                  <td className="py-2 px-4 border-b">
